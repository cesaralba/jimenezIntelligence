...
def _m_way_unordered_combinations(items, ks):
    ...
    if not any(ks[1:]):
        ...
        for c_first in itertools.combinations(items, ks[0]):
            ...
            for c_other in _m_way_unordered_combinations(items_remaining, ks[1:]):
...
def _unlabeled_balls_in_unlabeled_boxes(balls, box_sizes):
    ...
    if not balls:
        ...
        for balls_in_first_box in range(min(balls, box_sizes[0]), -1, -1):
            ...
            for distribution_other in _unlabeled_balls_in_unlabeled_boxes(balls_in_other_boxes, box_sizes[1:]):
